...
public class TestPetal extends PApplet {
	...
	final float kRotationSpeed = 0.0625f; // Revs per second
	...
	// Flower Colour
	int flowerCol = color(255, 0, 0, 40);
	int petalCol = color(200, 200, 25, 40);
	int stemCol = color(255, 255, 0, 40);
	...
	int interpolateColour(int startcol, int endcol, float nTime) {
	...
	void drawInstructions() {
		...
		if (!hideInstructions) {
			text("Number of Petals: '+' '-' keys", InstructionXPOS,
					InstructionYPOS + 1 * InstructionFontHeight);
			text("Petal Length: LEFT/RIGHT arrow keys", InstructionXPOS,
					InstructionYPOS + 2 * InstructionFontHeight);
			text("Petal Tilt: UP/DOWN arrow keys", InstructionXPOS,
					InstructionYPOS + 3 * InstructionFontHeight);
			text("Show/Hide Control Point Lines: 'C' key", InstructionXPOS,
					InstructionYPOS + 4 * InstructionFontHeight);
			text("Animate Petals and Tilt: 'A' key", InstructionXPOS,
					InstructionYPOS + 5 * InstructionFontHeight);
			text("Move Flower: 'M' key", InstructionXPOS, InstructionYPOS + 6
					* InstructionFontHeight);
			text("Show/Hide Stem Flower: 'S' key", InstructionXPOS,
					InstructionYPOS + 7 * InstructionFontHeight);
			text("Toggle Interpolate Colours: 'I' key", InstructionXPOS,
					InstructionYPOS + 8 * InstructionFontHeight);
			text("Motion Blur: 'B' key", InstructionXPOS, InstructionYPOS + 9
					* InstructionFontHeight);
			text("Stroke/NoStroke: 'N' key", InstructionXPOS, InstructionYPOS
					+ 10 * InstructionFontHeight);

			text("Reset: 'R' key", InstructionXPOS, InstructionYPOS + 11
					* InstructionFontHeight);
		}
		if (hideInstructions) {
			textFont(font, 16);
	...
	void drawInfoHud() {
		...
		if (!hideInstructions) {
			...
			text("Length: " + petalLength, InstructionXPOS, InstructionYPOS + 1
					* InstructionFontHeight);
			text("Tilt: " + tilt, InstructionXPOS, InstructionYPOS + 2
					* InstructionFontHeight);
			text("Animation: " + animation, InstructionXPOS, InstructionYPOS
					+ 4 * InstructionFontHeight);
			text("Movement: " + movement, InstructionXPOS, InstructionYPOS + 5
					* InstructionFontHeight);
		}
	...
	// four Vector points
	void drawPetal(PVector[] cps, int col) {
		fill(col);
		if (noStrk) {
			noStroke();
		} else {
		...
		bezier(cps[0].x, cps[0].y, cps[1].x, cps[1].y, cps[2].x, cps[2].y,
				cps[3].x, cps[3].y);
	}
	...
	PVector[] calcControlPoints(float angle, float ro, float radius,
			float tilt, float petalLength) {

		PVector[] bezControlPoints = new PVector[4];

		float halfRo = ro / 2.0f;

		PVector[] cPoints = calcPetalPointPair(angle - halfRo, tilt,
				petalLength);
		PVector startPt = cPoints[0];
		PVector fPoint = cPoints[1];

		bezControlPoints[0] = new PVector(startPt.x * radius, startPt.y
				* radius);
		bezControlPoints[1] = new PVector(startPt.x * radius + fPoint.x,
				startPt.y * radius + fPoint.y);

		PVector[] cPoints2 = calcPetalPointPair(angle + halfRo, -tilt,
				petalLength);
		startPt = cPoints2[0];
		fPoint = cPoints2[1];

		bezControlPoints[3] = new PVector(startPt.x * radius, startPt.y
				* radius);
		bezControlPoints[2] = new PVector(startPt.x * radius + fPoint.x,
				startPt.y * radius + fPoint.y);
	...
	PVector[] calcPetalPointPair(float angle, float tilt, float petalLength) {
		PVector startPt = new PVector(sin(degreesToRadians(angle)),
				cos(degreesToRadians(angle)));
		PVector vert = new PVector(0, 0, 1.0f); // Unit Vector Perp to XY Plane
		PVector nCPc = startPt.cross(vert); // Calculate Tangent to Circle at
											// startPt.x,startPt.y
		PMatrix2D iMat = new PMatrix2D();
		iMat.rotate(degreesToRadians(tilt < 0 ? 180 + tilt : tilt)); // Petal
																		// Tilt
		PMatrix2D cMat = new PMatrix2D(nCPc.x, startPt.x, 0.0f, nCPc.y,
				startPt.y, 0.0f);
		// Matrix
		// | nCPc.x startx |
		// | nCPc.y starty |
		cMat.preApply(iMat); // Apply our orthogonal axis to the rotation matrix
	...
	// sinFunction,cosFunction - Helper functions for auto animation of
	// Parameter values
	...
	float cosFunction(float startValue, float endValue, float timeF,
	...
	float sinFunction(float startValue, float endValue, float timeF,
			float period, float phase) {
		float yI = (startValue + endValue) / 2;
		float freq = 1.0f / period;
		float amp = endValue - yI;
	...
	void drawFlower(float stemRadius, float stepSize, float numPetals,
			float roAng, float tiltAng, float thtAng, float petalLen, int col) {
		...
		for (float angle = 0.0f; angle < stepSize * numPetals; angle += stepSize) {
			PVector[] cps = calcControlPoints(angle + thtAng, roAng,
					stemRadius, tiltAng, petalLen);
			int iCol;
			if (interpColours) {
				int endcol = color(blue(col), green(col), red(col), alpha(col));
	...
	public void draw() {
		...
		float dt = 1 / frameRate; // We should calculate dt - time passed since
									// last update-draw
		if (movement) {
			moveTime += dt;
		} else {
		...
		translate(
				CX
						+ sinFunction(-movementAmplitude, movementAmplitude,
								moveTime, 10.0f, 0.0f),
				CY
						+ sinFunction(movementAmplitude, -movementAmplitude,
								moveTime, 20.0f, 0.0f));
		drawFlower(radius, step, numpetals, ro, tilt, tht, petalLength,
				flowerCol);
		if (stemFlower) {
			drawFlower(radius, 720.0f / numpetals, numpetals / 2, ro, tilt,
					(float) (tht + 20.0), petalLength / 4, petalCol);
		}
		popMatrix();
		...
		if (animation) {
			animTime += dt;
			tilt = sinFunction(kLowestTilt, kHighestTilt, animTime,
					kTiltPeriod, 0.0f);
			numpetals = sinFunction(kLowestPetals, kHighestPetals, animTime,
					kPetalPeriod, 0.0f);
			numpetals = Math.max(1, numpetals);
			step = 360.0f / numpetals;
			// petalLength =
			// sinFunction(40.0f,260.0f,animTime,kPetalPeriod,0.0f) ;
		}
	}
