...
/**
 * JSMinim is an implementation of the {@link MinimServiceProvider} interface
 * that use Javasound to provide all audio functionality. That's about all you
...
public class JSMinim implements MinimServiceProvider {
	...
	private Method createInputFromFile;
	private Method dataPath;
	...
	public JSMinim(Object parent) {
		...
		try {
			...
			createInputFromFile = parent.getClass().getMethod("createInput", File.class);
			dataPath = parent.getClass().getMethod("dataPath", String.class);
			...
			if (createInputFromFile.getReturnType() != InputStream.class) {
				error += "The method createInput in the file loading object provided does not return an InputStream!\n";
				createInputFromFile = null;
			}

			if (dataPath.getReturnType() != String.class) {
				error += "The method dataPath in the file loading object provided does not return a String!\n";
				dataPath = null;
			}
	...
	public SampleRecorder getSampleRecorder(Recordable source, String fileName, boolean buffered) {
		...
		try {
			...
			if (buffered) {
				recorder = new JSBufferedSampleRecorder(this, destPath, fileType, source.getFormat(),
						source.bufferSize());
			} else {
				recorder = new JSStreamingSampleRecorder(this, destPath, fileType, source.getFormat(),
						source.bufferSize());
			}
		} catch (Exception ex) {
	...
	public AudioSample getAudioSample(String filename, int bufferSize) {
		...
		if (ais != null) {
			...
			AudioOut out = getAudioOutput(format.getChannels(), bufferSize, format.getSampleRate(),
	...
	private JSAudioSample getAudioSampleImp(FloatSampleBuffer samples, AudioFormat format, int bufferSize) {
		AudioOut out = getAudioOutput(samples.getChannelCount(), bufferSize, format.getSampleRate(),
				format.getSampleSizeInBits());
		if (out != null) {
			SampleSignal ssig = new SampleSignal(samples);
			out.setAudioSignal(ssig);
			...
			try {
				...
				if (is != null) {
					...
					if (ais != null) {
						...
						// will cause out-of-memory problems with very large
						// files.
					...
					if (Boolean.parseBoolean(System.getProperty("fromEclipse"))) {
						String m_filePath = (String) dataPath.invoke(fileLoader, filename);
						int index = m_filePath.indexOf("\\data\\");
						m_filePath = m_filePath.substring(0, index) + "\\src\\" + m_filePath.substring(index + 1);
						File sample = new File(m_filePath);
						if (sample.exists()) {
							InputStream is2 = (InputStream) createInputFromFile.invoke(fileLoader, sample);
							if (is2 != null) {
								debug("Base input stream is: " + is2.toString());
